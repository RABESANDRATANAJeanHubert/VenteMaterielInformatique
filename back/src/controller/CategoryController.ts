...
export class CategoryController {
  static add = async (req: any, res: any) => {
    try {
      ...
      return res
        .status(200)
        .send(
          helper.ResponseData(200, "Category has been created", null, category)
        );
    } catch (error) {
      return res
        .status(500)
    ...
    try {
      ...
      if (isEmpty(category)) {
        return res
          .status(201)
          .send(
            helper.ResponseData(201, "Information is empty", null, category)
          );
      }
      return res
        .status(200)
        .send(helper.ResponseData(200, "Category list", null, category));
    } catch (error) {
      return res
        .status(500)
        .send(
  ...
  static delete = async (req: any, res: any) => {
    try {
      ...
      return res
        .status(400)
        .send(helper.ResponseData(200, "Category has been delete", null, null));
    } catch (error) {
      return res
        .status(500)
        .status(
    ...
    try {
      if (isUndefined(name) || isEmpty(name)) {
        return res
          .status(400)
          .send(
            helper.ResponseData(400, "Information is required", null, null)
          );
      }
      ...
      if (!category) {
        return res
          .status(400)
          .send(helper.ResponseData(400, "name is not defined", null, null));
      }
      return res
        .status(200)
        .send(helper.ResponseData(200, "ok", null, category));
    } catch (error) {
      return res
        .status(500)
